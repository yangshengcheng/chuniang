...
class ModelTotalKlarnaFee extends Model {
	public function getTotal(&$total_data, &$total, &$taxes, $type=AllProduct) {
		...
		if (($this->cart->getSubTotalWithProducts($products) < $this->config->get('klarna_fee_total')) 
		&& ($this->cart->getSubTotalWithProducts($products) > 0) 
		&& isset($this->session->data['payment_method']['code']) 
		&& $this->session->data['payment_method']['code'] == 'klarna_invoice' || $this->session->data['payment_method']['code'] == 'klarna_pp') 
		{						
