...
class Customer {
  	...
  	public function __construct($registry) {
		...
		if (isset($this->session->data['customer_id'])) { 
			...
			if ($customer_query->num_rows) {
				...
				$this->friend_account = $customer_query->row['friend_account'];
	...
	public function login_telephone($telephone, $password, $override = false) {
		if ($override) {
			//$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer where LOWER(email) = '" . $this->db->escape(strtolower($email)) . "' AND status = '1'");
			$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer where LOWER(telephone) = '" . $this->db->escape(strtolower($telephone)) . "' AND status = '1'");
		} else {
			$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(telephone) = '" . $this->db->escape(strtolower($telephone)) . "' AND password = '" . $this->db->escape(md5($password)) . "' AND status = '1' AND approved = '1'");
		}
